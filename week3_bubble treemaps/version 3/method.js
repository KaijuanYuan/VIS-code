...
var pointTangent;//the tagent point of two circles
...
//find the tangent point of two circles
function tangent(circle1, circle2, loopDirection)
{
	//将交点坐标替换成smoothness圆
	var a1,b1,r1;
	var a2,b2,r2;
...
*/
	...
	for(var i=1; i<R.length-1; i++)
	{
		...
		{
			startPoint = Math.atan( (R[i-1].y - R[i].y) / (R[i-1].x - R[i].x) );
			if(startPoint < 0)
			{
				//第一象限
				if((R[i-1].y - R[i].y)<0)
			...
			{
				//第二象限
				if((R[i-1].x - R[i].x)<0 && (R[i-1].y - R[i].y)<0)
				{
					startPoint = Math.PI + Math.abs(startPoint);
				}
				//第四象限
			...
			endPoint = Math.atan( (R[i+1].y - R[i].y) / (R[i+1].x - R[i].x) );
			if(endPoint < 0)
			{
				//第一象限
				if((R[i+1].y - R[i].y)<0)
			...
			if(i/2 %2 ==0)
			{
				ctx.beginPath();
				ctx.strokeStyle = "#FF0000";
				ctx.arc(R[i].x, R[i].y, R[i].radius, startPoint, endPoint);
				ctx.stroke();
			}
			else
			{
				ctx.beginPath();
				ctx.strokeStyle = "#FF0000";
				ctx.arc(R[i].x, R[i].y, R[i].radius, endPoint, startPoint);
				ctx.stroke();
			}
	...
	//画出第一个点的弧
	var startPoint = Math.atan( (R[R.length-2].y - R[0].y) / (R[R.length-2].x - R[0].x) );
	if(startPoint < 0)
	{
		//第一象限
		if((R[R.length-2].y - R[0].y)<0)
		{
			startPoint = 2 * Math.PI - Math.abs(startPoint);
		}
		//第三象限
		else if((R[R.length-2].x - R[0].x)<0)
		{
			startPoint = Math.PI - Math.abs(startPoint);
		}
	...
	{
		//第二象限
		if((R[R.length-2].x - R[0].x)<0 && (R[R.length-2].y - R[0].y)<0)
		{
			startPoint = Math.PI + Math.abs(startPoint);
		}
		//第四象限
	...
	var endPoint = Math.atan( (R[1].y - R[0].y) / (R[1].x - R[0].x) );
	if(endPoint < 0)
	{
		//第一象限
		if((R[1].y - R[0].y)<0)
	...
	{
		//第二象限
		if((R[1].x - R[0].x)<0 && (R[1].y - R[0].y)<0)
		{
			endPoint = Math.PI + Math.abs(endPoint);
		}
		//第四象限
	...
	ctx.beginPath();
	ctx.strokeStyle = "#FF0000";
	ctx.arc(R[0].x, R[0].y, R[0].radius, startPoint, endPoint);
	ctx.stroke();
